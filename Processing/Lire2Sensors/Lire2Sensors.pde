...
int[] serialInValVec = new int[2];    // Where we'll put what we receive
...
void draw() {
  ...
  float noiseXRange = serialInValVec[0]/50.0; // serialInVal/50.0;
  float noiseYRange = serialInValVec[1]/50.0;// serialInVal/50.0;      //mouseY/100.0;       //2*(serialInVal-100); 
  //pour quoi *2?
...
void serialEvent(Serial myPort) {
  ...
  if (firstContact == false) {
    ...
    if (inByte !=0){
      serialInValVec[serialCount] = inByte;
      serialCount = (serialCount+1)%2;
    }
    print("read: ");
    println(serialInValVec);
    myPort.write('A');
  }
}